    ...
    <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4 group">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="ml-auto flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => { setEditing(!editing); setEditContent(block.content); }}
      ...
      {editing ? (
        <div className="space-y-2">
          ...
          />
          <button
            onClick={() => { onEdit(index, editContent); setEditing(false); }}
...
function PreviewPane({ blocks, title }) {
  return (
    <div className="bg-white/[0.02] border border-slate-700/50 rounded-xl p-6 overflow-y-auto max-h-[600px]">
      ...
      {blocks.length === 0 && (
        <p className="text-slate-600 text-center py-12">Paste text and click Parse to see preview</p>
      )}
    </div>
  );
}
...
export default function AdminContentStudio({ adminToken }) {
  ...
  const handleParse = useCallback(async () => {
    ...
    try {
      ...
      if (!title && parsed.length > 0) {
        setTitle(parsed[0].title || subject || 'Untitled');
      }
  ...
  const handleEditBlock = (index, newContent) => {
    setBlocks(prev => prev.map((b, i) => i === index ? { ...b, content: newContent } : b));
  };
    ...
    <div className="p-6 space-y-5">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-slate-500 text-xs">
                Structured Blocks {blocks.length > 0 && `(${blocks.length})`}
              </label>
              {blocks.length > 0 && (
                <button
                  onClick={() => setBlocks([])}
      ...
      {blocks.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <h3 className="text-slate-300 text-sm font-semibold mb-4">Publish to SEO Pages</h3>
