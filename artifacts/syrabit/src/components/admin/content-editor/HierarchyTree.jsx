        ...
        {boards.map(b => (
          <div key={b.id}>
            <div className="flex items-center group">
              ...
              <button onClick={() => onDelete('board', b.id)} className="p-1 rounded opacity-0 group-hover:opacity-100 text-white/20 hover:text-red-400"><Trash2 size={12} /></button>
            </div>
                ...
                {filteredClasses.map(c => (
                  <div key={c.id}>
                    <div className="flex items-center group">
                      ...
                      <button onClick={() => onDelete('classe', c.id)} className="p-1 rounded opacity-0 group-hover:opacity-100 text-white/20 hover:text-red-400"><Trash2 size={10} /></button>
                    </div>
                      ...
                      <div className="ml-4 mt-1 space-y-1 border-l border-white/5 pl-3">
                        ...
                        {filteredStreams.map(st => (
                          <div key={st.id} className="flex items-center group">
                            ...
                            <button onClick={() => onDelete('stream', st.id)} className="p-1 rounded opacity-0 group-hover:opacity-100 text-white/20 hover:text-red-400"><Trash2 size={10} /></button>
                          </div>
                        ))}
