...
# Task #563: the production-importable implementation now lives in
# `seo_indexnow_smoke.py` so the admin route / cron loop can call it
# without dragging in `install_deps_stub()`. We re-export it here so
# any existing pytest references keep working unchanged.
from seo_indexnow_smoke import run_publish_indexnow_smoke  # noqa: E402,F401
