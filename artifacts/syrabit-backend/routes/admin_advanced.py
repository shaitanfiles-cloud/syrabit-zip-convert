...
async def _build_llms_txt() -> str:
    lines = [
        ...
        "",
        "- Zero-Hallucination AI: 6-stage RAG pipeline with relevance gating (thresholds 0.55–0.65). If no syllabus content matches with high confidence, Syra declines rather than hallucinating.",
        "- 5-Provider LLM Failover: Sarvam AI → Cerebras → Gemini → Fireworks → OpenRouter. No single point of AI failure. All 5 providers must fail simultaneously for downtime.",
        "- Syllabus-Native Architecture: Board → Class/Semester → Stream/Course → Subject → Chapter → Topic. Supports NEP FYUGP course types (AEC, SEC, MDC, VAC, GE) that most platforms don't know exist.",
        "- Chat-to-Content Pipeline: Admin promotes high-quality AI chat answers into permanent QA pairs → auto-grouped into FAQ SEO pages with Schema.org markup.",
        "- Real-Time Web Search Fallback: Custom crawler (SyrabitBot) with Playwright + Trafilatura. Can fetch and extract text from online PDFs during conversations.",
        ...
        "- Intelligent Analytics: Custom session tracking (visitor_id, session_id), bounce rates, content gap identification via interaction events (ask_ai_click, subject_view, search_query).",
        "- Sarvam AI Voice & Indian Language Support: TTS via Bulbul v2 (40+ speakers, Assamese/Hindi/English), translation via Mayura v1 (10+ Indian languages), transliteration across Indian scripts.",
        "",
        "## Pricing & Access Model",
        "",
        "Freemium credit system designed for Indian students:",
        "",
        "| Plan | Price | Daily AI Credits | Rate Limit | Max Tokens | Document Access |",
        "| --- | --- | --- | --- | --- | --- |",
        "| Free | ₹0 | 30 | 5 msg/min | 10,000 | Limited |",
        "| Starter | ₹99 (one-time) | 500 | 10 msg/min | 15,000 | Full |",
        "| Pro | ₹999 (one-time) | 4,000 | 15 msg/min | 20,000 | Full |",
        "",
        "All study notes, MCQs, definitions, and examples are freely accessible without login. AI tutor requires credits. Payments via Razorpay (UPI, cards, net banking) in INR.",
        "",
        "## Geographic Focus",
        "",
        ...
        "",
        "### Stack",
        "- Frontend: React 19, Vite, Tailwind CSS, PWA (offline-capable, installable)",
        "- Backend: Python 3, FastAPI, Gunicorn (multi-worker async)",
        "- Databases: MongoDB (content, syllabi, embeddings), PostgreSQL (users, sessions, credits), Upstash Redis (cache, rate limits)",
        "- Auth: Google OAuth 2.0 (PKCE) — no passwords stored",
        "- Payments: Razorpay (UPI, cards, net banking — INR pricing with HMAC-SHA256 webhook verification)",
        ...
        "2. Content Retrieval: Vector similarity search with relevance gating (threshold 0.55-0.65) — below-threshold queries are declined, never hallucinated",
        "3. Reranking: Voyage AI reranking model prioritizes exam-relevant sections",
        "4. Topic-Aware Extraction: SequenceMatcher-based fuzzy heading matcher extracts the precise topic section from chapter content",
        ...
        "",
        "### Multi-Provider LLM Failover (SLM Pool)",
        "Ordered cascade with automatic failover — students never see a hard failure:",
        "1. Sarvam AI (Sarvam-M) — India-optimized, low latency",
        "2. Cerebras (Llama 3.1 8B) — ultra-fast inference",
        "3. Google Gemini (Gemini 2.5 Flash) — high-quality, fast throughput",
        "4. Fireworks AI (DeepSeek V3) — deep reasoning for complex questions",
        "5. OpenRouter (DeepSeek Chat V3) — final fallback",
        "Each slot has independent timeouts (10s TTFT, 8s per-slot). Admin content generation uses reversed priority.",
        "",
        "### Sarvam AI Voice & Language Services",
        "- TTS: Bulbul v2 model with 40+ Indian voice speakers (Assamese, Hindi, English). Configurable pace, pitch, loudness. Audio cached as base64.",
        "- Translation: Mayura v1 for specific Indian languages, Sarvam-Translate v1 for general coverage (10+ Indian languages).",
        "- Transliteration: Script conversion between Devanagari, Assamese, Bengali, and Latin scripts.",
        "- LLM: Sarvam-M (India-optimized model, 1st in failover chain). Better comprehension of mixed-language queries (English + Assamese/Hindi).",
        "- All Sarvam requests use persistent HTTP/2 connection pools (connect: 3s, read: 30s). Optional Cloudflare AI Gateway routing for metrics/caching.",
        "",
        "### Performance Optimizations",
        "- Multi-layer caching: In-memory TTLCache (L1, 5-60min) → Redis (L2) → CDN (L3) → Service Worker (L4, network-first for HTML, cache-first for images)",
        "- Frontend: React.lazy() code splitting, manual Vite chunk splitting (vendor, router, query, motion, radix), requestIdleCallback prefetching",
        "- AI streaming: Server-Sent Events (SSE) for real-time token delivery. Time-to-first-token: 1-3 seconds.",
        "- Parallel fetching: asyncio.gather for RAG context + web search + conversation history (40-60% latency reduction vs sequential)",
        "- GZip compression on all responses >500 bytes. WebP images for 30-50% size reduction.",
        "- React Query: 5-min stale time, 60-min GC — eliminates redundant API calls between page navigations.",
        "- Database indexing: Critical MongoDB indexes (text search, unique constraints, compound keys) created on startup.",
        "",
        ...
        "",
        "### Security",
        "- CSP, X-Frame-Options, HSTS, referrer-policy headers on all responses",
        "- Razorpay webhook HMAC-SHA256 verification",
        "- Per-user credit tracking with daily rate limits via Redis",
...
async def _pipeline_process_one_chapter(
    ...
    *,
    ...
    async with semaphore:
        ...
        if generated_notes:
            try:
                await auto_chunk_content(chapter_id=chapter_id, content=generated_notes, subject_id=subject_id, category="notes", topics=ch_topics)
    ...
    async for ch in thin_cursor:
        ...
        try:
            ...
            if new_wc >= 600:
                ...
                chunks = await auto_chunk_content(ch["id"], new_content, ch.get("subject_id"), category=ch.get("category", "notes"), topics=ch.get("topics"))
