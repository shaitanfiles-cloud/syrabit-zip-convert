...
_google_reviews_cache: dict = {"data": None, "fetched_at": 0.0}
_GOOGLE_REVIEWS_TTL = 6 * 3600
_GOOGLE_REVIEWS_NEG_TTL = 300

def _parse_redis_json(val):
    if val is None:
        return None
...
@router.get("/content/google-reviews")
async def get_google_reviews(response: Response = None):
    from config import GOOGLE_PLACES_API_KEY, GOOGLE_PLACE_ID

    if not GOOGLE_PLACES_API_KEY or not GOOGLE_PLACE_ID:
        return {"reviews": [], "configured": False, "place_id": ""}

    now = time.time()
    if _google_reviews_cache["data"] and (now - _google_reviews_cache["fetched_at"]) < _GOOGLE_REVIEWS_TTL:
        if response:
            response.headers["Cache-Control"] = "public, max-age=1800, stale-while-revalidate=21600"
        return _google_reviews_cache["data"]

    if redis_client:
        try:
            val = redis_client.get("google_reviews")
            if val is not None:
                parsed = _parse_redis_json(val)
                if isinstance(parsed, dict):
                    _google_reviews_cache["data"] = parsed
                    _google_reviews_cache["fetched_at"] = now
                    if response:
                        response.headers["Cache-Control"] = "public, max-age=1800, stale-while-revalidate=21600"
                    return parsed
        except Exception:
    ...
    _empty_result = {"reviews": [], "configured": True, "place_id": GOOGLE_PLACE_ID}
    ...
    try:
        ...
        if data.get("status") != "OK":
            ...
            _google_reviews_cache["data"] = _empty_result
            _google_reviews_cache["fetched_at"] = now - _GOOGLE_REVIEWS_TTL + _GOOGLE_REVIEWS_NEG_TTL
            return _empty_result

        result_data = data.get("result", {})
        raw_reviews = result_data.get("reviews", [])
        ...
        result = {
            ...
            "configured": True,
            "place_id": GOOGLE_PLACE_ID,
            "overall_rating": result_data.get("rating"),
            "total_ratings": result_data.get("user_ratings_total"),
        }

        _google_reviews_cache["data"] = result
        _google_reviews_cache["fetched_at"] = now

        if redis_client:
            try:
                redis_client.set("google_reviews", json.dumps(result, default=str), ex=_GOOGLE_REVIEWS_TTL)
            except Exception:
                pass
