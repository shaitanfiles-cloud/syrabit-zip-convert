...
async def _edge_proxy_deploy_cron_alert_loop():
    ...
    Mongo-backed lease so only one replica polls upstream state on
    each tick. This keeps the GitHub REST quota usage at 1× regardless
    of replica count. Followers stand down on each tick.
    """
    from deps import db, is_mongo_available  # type: ignore
    import background_lease as _bglease
    owner_id = _bglease.make_owner_id("edge-proxy-deploy-cron")
    lock_id = "edge_proxy_deploy_cron_alert_lease"
