...
- **Frontend:** `artifacts/syrabit/src/components/admin/AdminLogsExplorer.jsx` — registered in `AdminPage`. Filter bar covers all server-side filters; live-tail polls `GET /api/admin/logs` every 5s with the newest seen timestamp as `since`; export buttons hit `/api/admin/logs/export?fmt=csv|ndjson` via `adminLogsDownloadExport` (axios blob fetch with the admin Bearer token in `Authorization`, then a synthetic `<a download>` click) so the download works in environments that don't have an admin cookie; falls back to `window.open(adminLogsExportUrl(...))` if the blob path errors. Clicking a row's correlation badge opens the trace pane via `/trace/{cid}`.
- **Tests (61 total, all green):** `tests/test_unified_logs_dao.py` (18) — normalisation, sampling, indexes; `tests/test_admin_logs_route.py` (13) — auth gates, filters, exports, ingest token, pause; `tests/test_unified_logs_cf_pull.py` (5) — CF GraphQL row mapping, cursor advance, no-config short-circuit. Worker: `workers/edge-proxy/tests/log-shipper.test.ts` (20) — batching, sampling, token rotation, waitUntil non-blocking. Frontend: `AdminLogsExplorer.test.jsx` (5) — filter→fetch wiring, live-tail incremental fetch, trace pane, export URL shape.
