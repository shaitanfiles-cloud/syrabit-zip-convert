...
- **Cloudflare Edge Cache Auto-Purge:** Utilizes Cloudflare Cache Purge API for invalidating cached content. Worker Cache API purge endpoint (`POST /api/edge/purge`) clears CF Workers Cache entries by prefix or all, triggered automatically alongside zone-level purge on content invalidation. Library bundle supports `?slim=1` mode returning boards/classes/streams/subjects without chapters for faster initial page loads. Pipeline result caching: notes, mark-wise PYQs, topic PYQs, and flashcards cached in Redis (2h TTL) keyed by content hash to skip duplicate LLM calls.
- **IndexNow Integration:** Backend endpoints `POST /admin/indexnow/ping` and `GET /admin/indexnow/status` for instant URL submission to search engines. Static key file at `/syrabit-indexnow-2026-key.txt`. Submits up to 10,000 URLs per batch via IndexNow API.