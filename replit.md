...
- **Optional Authentication:** Chat, History, and Profile pages are accessible to anonymous users via a `syrabit_anon_id`. Anonymous conversations are persisted to both Upstash Redis (7-day TTL for fast reads) and PostgreSQL (`conversations` table with `is_anonymous=TRUE`, `anon_id` columns) for permanent storage. Read paths fall back to PostgreSQL when Redis TTL expires. Anonymous chat messages are also logged to MongoDB for analytics.
...
- **PWA:** Fully optimized with a multi-cache service worker (v7), precached icons, cache trimming, and offline fallback. Hashed JS/CSS assets use cache-first strategy (immutable). Navigation uses stale-while-revalidate for instant repeat loads with background updates. PostHog analytics deferred to requestIdleCallback. Tracks installations via MongoDB.
...
- **Production Deployment:** Hybrid architecture — FastAPI backend on Render Starter ($7/mo, always-hot), Cloudflare Worker edge proxy (`api.syrabit.ai`) for caching/rate-limiting, frontend on Cloudflare Pages (`syrabit.ai`). All API traffic routes through the Worker edge proxy at `https://api.syrabit.ai` which forwards to Render. Frontend uses `VITE_BACKEND_URL=https://api.syrabit.ai` as the single API endpoint. `VITE_WORKER_API_URL` and `VITE_RENDER_API_URL` fall back to `VITE_BACKEND_URL` when not set. Cookie domain is `.syrabit.ai` for cross-subdomain auth. Worker config in `workers/edge-proxy/`, Render config in `artifacts/syrabit-backend/render.yaml`, deployment guide in `docs/DEPLOYMENT.md`.