...
- **Edge worker shipper:** `workers/edge-proxy/src/log-shipper.ts` collects per-request log entries in an in-memory ring buffer and flushes them via `ctx.waitUntil(fetch(BACKEND_INGEST_URL, …))` so the user-facing response is never blocked. Authenticates with the rotated `X-Logs-Ingest-Token` from worker secret storage. Wrapped in `index.ts` so every fetch handler emits an entry with method, path, status, duration, country, colo, cache status, and `cf-ray` (which becomes both `ray_id` and `correlation_id` for that record — the only producer that has a per-request ray identifier).
- **Frontend:** `artifacts/syrabit/src/components/admin/AdminLogsExplorer.jsx` — registered in `AdminPage`. Filter bar covers all server-side filters; live-tail polls `GET /api/admin/logs` every 5s with the newest seen timestamp as `since`; export buttons hit `/export?fmt=csv|ndjson`; clicking a row's correlation badge opens the trace pane via `/trace/{cid}`.
