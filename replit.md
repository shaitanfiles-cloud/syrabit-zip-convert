...
- **Analytics:** Dual-destination tracking (GA4 + PostHog) for various user events. Server-side `ServerSideTrackingMiddleware` logs every non-asset page request to MongoDB `server_hits` collection with hashed IP (daily + stable), user-agent, bot detection, and country. The admin dashboard displays three traffic tiers: "All Traffic" (server-side, Cloudflare-equivalent), "Engaged Visitors" (JS-tracked), and "Bot/Crawler Traffic" (separate). IP deduplication uses SHA-256 hashed IPs with a configurable salt (`IP_HASH_SALT` env var).
