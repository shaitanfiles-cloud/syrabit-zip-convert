...
- **Edge Caching Strategy:** Cloudflare Worker caches 20+ public/read-only GET routes (content, notes, MCQs, flashcards, CMS articles, sitemaps) with long-lived TTLs (1 week / 604800s for content, shorter for SEO/user-stats). Content routes are cached even when `Authorization`/`Cookie`/`x-anon-id` headers are present (only user-specific routes like `/api/user/stats` bypass cache with auth). Dynamic routes (`/api/ai/chat/*`, `/api/webhooks/*`, `/api/auth/*`) and all non-GET methods bypass cache and proxy directly to Railway. SSE streaming for AI chat passes through untouched.
