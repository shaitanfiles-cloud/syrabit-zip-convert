...
- **Chat Latency:** Concept queries: TTFT ~0.7s, total ~0.9s (web search skipped). PYQ/current queries: TTFT ~1.7s, total ~2.2s (web search fires). SLM pool TTFT timeout: 1.0s. Stage 1 timeout: 0.8s. Search scope timeout: 0.35s (fire-and-forget). Web search timeout: 1.5s (DDG timeout: 1.2s). Default model: `meta-llama/llama-4-scout-17b-16e-instruct` (Groq). Max tokens: free=1024, starter=1536, pro=2048.
- **Smart Web Search Gating:** Stage 1 classifies `needs_web_search` (true/false) to skip web search for basic concept queries the LLM knows. Heuristic fallback (`_SIMPLE_Q_RE`, `_WEB_NEEDED_RE`) handles Stage 1 timeouts. Safety net: intents `pyq`, `important_questions`, `syllabus`, `chapter_meta` always fire web search regardless.
- **Response Length:** Concise by default (50-100 words). max_tokens capped per plan. Prompt rules enforce brevity: "what is X?" → 2-3 sentences, "explain X" → 4-6 sentences.
