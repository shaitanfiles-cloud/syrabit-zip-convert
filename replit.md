...
- **Pipeline (RAG + Web Search removed):** Stage 1 topic metadata (subject/chapter classification) is preserved for prompt context. Document uploads still use `resolve_rag_context` for document-only grounding. Web search (DDG) fully removed — was consistently blocked/failing on production with 0% success rate. Pipeline is now: Stage 1 classify → LLM (no web search layer).
...
- **Chat Latency:** English queries: total ~1.0-1.1s (Stage 1 0.8s + LLM ~0.3s). Assamese: ~1.4-2.1s. Stage 1 timeout: 0.8s (0.4s for Assamese). Default model: `meta-llama/llama-4-scout-17b-16e-instruct` (Groq). Max tokens: free=512, starter=768, pro=1024.
