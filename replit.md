...
- **Multi-LLM Pipeline (`pipeline.py`):** A 3-stage chain that improves answer quality by separating concerns: (1) **Topic Resolver** — fast/small model (Cerebras llama3.1-8b or Groq) extracts structured topic metadata (subject, chapter, topic, intent, search keywords) as JSON, replacing regex-based intent classification for academic queries; (2) **RAG Synthesizer** — mid-tier model (Groq llama-3.3-70b or Fireworks) reads RAG chunks and produces a strictly-grounded factual draft; (3) **Response Polisher** — premium model (Gemini Flash via SLM pool) formats the draft into a student-friendly response with headings, examples, and clear language. Stage 3 streams to the user. Casual/simple queries bypass the pipeline. If any stage fails, graceful fallback to the existing single-LLM approach ensures the user always gets an answer. Per-stage timing and metrics are tracked via `get_pipeline_stats()`.
