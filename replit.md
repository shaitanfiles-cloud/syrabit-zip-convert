...
- **Performance Optimizations:** Library-bundle query prefetched on app mount (1s delay). CMS sections (CmsDocsSection, CmsPostsGrid) lazy-loaded via intersection observer + React.lazy(). CMS data uses React Query for cross-navigation caching. MasonryInfiniteGrid replaced with CSS grid + "Load more" button. Nav items preload page chunks on hover/touch via `pageImports` utility. Vite manualChunks splits lucide-react and @radix-ui into separate chunks. Subject thumbnail images use `<img loading="lazy">`.
