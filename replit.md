...
- **AI-Powered Syllabus Uploader:** An agentic pipeline handles PDF uploads, extracts syllabus information using Gemini Vision, generates board-aware LLM notes via `call_llm_api()`, chunks content for RAG, embeds chapters, creates CMS blog drafts, and performs SEO/GEO tagging. This process streams SSE events to the frontend for real-time progress. Content generation uses quality gates (retry once if <500 words, flag needs_review). Tested with Commerce SEC FYUGP PDF — 3 subjects, 12 chapters, 679 chunks, 74 embeddings.
- **Test Suite:** 92 pytest tests across 4 files in `artifacts/syrabit-backend/tests/` covering board normalization (35), chunking (16), prompts (29), and RAG pipeline (12).
